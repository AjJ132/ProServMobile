//
//  Calendar-Carousel.swift
//  ProServ Mobile
//
//  Created by AJ Johnson on 8/6/23.
//

import SwiftUI

struct Calendar_Carousel: View {
    var UIState: UIStateModel
    var cards: [Card]
    
    init(newDates: [Date], uiState: UIStateModel) {
        self.UIState = uiState
        var tempDates: [String] = []
        for item in newDates {
            let s = dayOfWeek(from: item)
            tempDates.append(s)
        }
        
        var tempCards: [Card] = []
        for (index, item) in tempDates.enumerated() {
            let c = Card(id: index, name: item)
            tempCards.append(c)
        }
    ...
    var body: some View {
        ...
        return Canvas {
            Carousel(numberOfItems: CGFloat(cards.count), spacing: spacing, widthOfHiddenCards: widthOfHiddenCards) {
